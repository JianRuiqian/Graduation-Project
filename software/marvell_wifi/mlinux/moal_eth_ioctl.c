...
static const t_u8 SupportedInfraBand[] = {
	BAND_B,
...
static const t_u8 SupportedAdhocBand[] = {
...
parse_arguments(t_u8 * pos, int *data, int datalen, int *user_data_len)
{
	...
	for (i = 0, j = 0, k = 0; i <= strlen((const char *)pos); i++) {
		if ((k == 0) && (i <= (strlen((const char *)pos) - 2))) {
...
woal_setget_priv_bandcfg(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_BANDCFG))) {
		/* GET operation */
		user_data_len = 0;
	} else {
		/* SET operation */
...
woal_setget_priv_httxcfg(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_HTTXCFG))) {
		/* GET operation */
		user_data_len = 0;
	} else {
		/* SET operation */
		memset((char *)data, 0, sizeof(data));
		parse_arguments(respbuf + strlen(CMD_MARVELL) +
				strlen(PRIV_CMD_HTTXCFG), (int *)data,
				ARRAY_SIZE(data), &user_data_len);
	}
...
woal_priv_delba(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) == header_len) {
	...
	mac_pos = (t_u8 *)strstr((const char *)respbuf + header_len, " ");
	if (mac_pos)
		mac_pos = (t_u8 *)strstr((const char *)mac_pos + 1, " ");
	if (mac_pos) {
#define MAC_STRING_LENGTH   17
		if (strlen((const char *)mac_pos + 1) != MAC_STRING_LENGTH) {
			PRINTM(MERROR, "%d: Invalid arguments\n", __LINE__);
			ret = -EINVAL;
			goto done;
		}
		woal_mac2u8(peer_mac, (char *)mac_pos + 1);
		*mac_pos = '\0';
	}

	parse_arguments(respbuf + header_len, (int *)data, ARRAY_SIZE(data),
			&user_data_len);
	...
	ret = sprintf((char *)respbuf, "OK. BA deleted successfully.\n") + 1;
...
woal_priv_rejectaddbareq(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) ==
		...
		memset((char *)data, 0, sizeof(data));
		parse_arguments(respbuf + strlen(CMD_MARVELL) +
				strlen(PRIV_CMD_REJECTADDBAREQ), (int *)data,
				ARRAY_SIZE(data), &user_data_len);
	}
	...
	if (req->action == MLAN_ACT_GET) {
		sprintf((char *)respbuf, "0x%x",
			cfg_11n->param.reject_addba_req.conditions);
		ret = strlen((const char *)respbuf) + 1;
	} else {
		ret = sprintf((char *)respbuf, "OK\n") + 1;
	}
...
woal_set_rx_ba_winsize(moal_private * priv, t_u8 * respbuf, int respbuflen)
{
	...
	memset((char *)data, 0, sizeof(data));
	if (respbuf && strlen((const char *)respbuf) > 0)
			   ...
			   t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) ==
		...
		memset((char *)data, 0, sizeof(data));
		parse_arguments(respbuf + strlen(CMD_MARVELL) +
				strlen(PRIV_CMD_TXRATECFG), (int *)data,
				ARRAY_SIZE(data), &user_data_len);
	}
			   ...
			   t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) ==
		...
		memset((char *)data, 0, sizeof(data));
		parse_arguments(respbuf + strlen(CMD_MARVELL) +
				strlen(PRIV_CMD_ESUPPMODE), (int *)data,
				ARRAY_SIZE(data), &user_data_len);
	}
			    ...
			    t_u32 respbuflen)
{
	...
	begin = (char *)respbuf + strlen(CMD_MARVELL) + strlen(PRIV_CMD_PASSPHRASE);
	...
	while (begin) {
		...
		if (!opt || !end || !end[0]) {
			...
			ret = -EINVAL;
			break;
		} else if (!strncmp(opt, "ssid", strlen(opt))) {
		...
		} else if (!strncmp(opt, "bssid", strlen(opt))) {
			woal_mac2u8((t_u8 *) & sec->param.passphrase.bssid,
				    end);
		} else if (!strncmp(opt, "psk", strlen(opt)) &&
			   req->action == MLAN_ACT_SET) {
		...
		} else if (!strncmp(opt, "passphrase", strlen(opt)) &&
			   req->action == MLAN_ACT_SET) {
	...
	if (sec->param.passphrase.ssid.ssid_len) {
		len += sprintf((char *)respbuf + len, "ssid:");
...
woal_priv_deauth(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) > (strlen(CMD_MARVELL) + strlen(PRIV_CMD_DEAUTH))) {
		/* Deauth mentioned BSSID */
		woal_mac2u8(mac,
			    (char *)respbuf + strlen(CMD_MARVELL) +
...
woal_priv_bssrole(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	memset((char *)data, 0, sizeof(data));
	if (strlen((const char *)respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_BSSROLE))) {
		/* GET operation */
		user_data_len = 0;
	} else {
		/* SET operation */
...
woal_priv_extcapcfg(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) == header)
		/* GET operation */
		req->action = MLAN_ACT_GET;
	else {
		/* SET operation */
...
woal_priv_setgetdeepsleep(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) ==
		...
		memset((char *)data, 0, sizeof(data));
		parse_arguments(respbuf + strlen(CMD_MARVELL) +
				strlen(PRIV_CMD_DEEPSLEEP), (int *)data,
				ARRAY_SIZE(data), &user_data_len);
	}
	...
	if (user_data_len == 0) {
		...
		sprintf((char *)respbuf, "%d %d", data[0], data[1]);
		ret = strlen((const char *)respbuf) + 1;
	} else {
...
woal_priv_setgetipaddr(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	data_length = strlen((const char *)respbuf) - header;
	...
	if (data_length < 1) {	/* GET */
		...
		in4_pton((const char *)&respbuf[header + 2],
		...
		if (woal_atoi(&op_code, (char *)&respbuf[header]) !=
		    MLAN_STATUS_SUCCESS) {
			ret = -EINVAL;
			goto done;
		}
	...
	if (req->action == MLAN_ACT_GET) {
		snprintf((char *)respbuf, IPADDR_MAX_BUF, "%d;%d.%d.%d.%d",
...
woal_priv_setwpssession(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) ==
		...
		/* SET operation */
		parse_arguments(respbuf + strlen(CMD_MARVELL) +
				strlen(PRIV_CMD_WPSSESSION), (int *)data,
				ARRAY_SIZE(data), &user_data_len);
	}
...
woal_priv_setgettcpackenh(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) ==
		...
		memset((char *)data, 0, sizeof(data));
		parse_arguments(respbuf + strlen(CMD_MARVELL) +
				strlen(PRIV_CMD_TCPACKENH), (int *)data,
				ARRAY_SIZE(data), &user_data_len);
	}
...
woal_priv_assocessid(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) ==
		...
		essid_length = strlen((const char *)essid_ptr);
	}
				 ...
				 t_u32 respbuflen)
{
	...
	if (req->action == MLAN_ACT_GET) {
		sprintf((char *)respbuf, "%d", pcfg_bss->param.listen_interval);
		ret = strlen((const char *)respbuf) + 1;
	} else {
		ret = sprintf((char *)respbuf, "OK\n") + 1;
	}
...
woal_priv_set_get_drvdbg(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_DRVDBG))) {
		/* GET operation */
		user_data_len = 0;
	} else {
		/* SET operation */
		...
		BOOLEAN invoke_hostcmd)
{
	...
	if (strlen((const char *)respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_HSCFG))) {
		/* GET operation */
		user_data_len = 0;
	} else {
		/* SET operation */
...
woal_priv_hssetpara(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (user_data_len >= 1 && user_data_len <= 3) {
		sprintf((char *)respbuf, "%s%s%s", CMD_MARVELL, PRIV_CMD_HSCFG,
			respbuf + (strlen(CMD_MARVELL) +
				   strlen(PRIV_CMD_HSSETPARA)));
		respbuflen = strlen((const char *)respbuf);
...
woal_priv_set_get_scancfg(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	memset(data, 0, sizeof(data));
	if (strlen((const char *)respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_SCANCFG))) {
		/* GET operation */
		user_data_len = 0;
	} else {
		/* SET operation */
...
woal_priv_pscfg(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) == header_len) {
		...
		arguments = kzalloc(strlen((const char *)respbuf) * sizeof(char), flag);
		if (arguments == NULL) {
			ret = -ENOMEM;
			goto done;
		}
		strcpy((char *)arguments, (const char *)respbuf + header_len);
		if (*(char *)arguments == '-') {
			is_negative_1 = MTRUE;
			arguments += sizeof(char);
		}
		space_ind = (t_u8 *)strstr((char *)arguments, " ");
		if (space_ind)
			space_ind = (t_u8 *)strstr((const char *)space_ind + 1, " ");
		if (space_ind) {
			if (*(char *)(space_ind + 1) == '-') {
				is_negative_2 = MTRUE;
				arguments[space_ind + 1 - arguments] = '\0';
				strcat((char *)arguments, (const char *)space_ind + 2);
			}
		}
		parse_arguments(arguments, data, ARRAY_SIZE(data),
				&user_data_len);
			      ...
			      t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) > header_len) {
		copy_len = MIN(strlen((const char *)data_ptr), sizeof(delts_ioctl));
...
woal_priv_fwmacaddr(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) == header_len) {
		...
		req->action = MLAN_ACT_SET;
		memset(data, 0, sizeof(data));
		woal_mac2u8(data, (char *)respbuf + header_len);
...
woal_priv_offchannel(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (header_len == strlen((const char *)respbuf)) {
 ...
 */
static int
woal_priv_net_monitor_ioctl(moal_private * priv, t_u8 * respbuf,
			    t_u32 respbuflen)
{
	int user_data_len = 0, header_len = 0;
	int data_length = 0;
...
woal_priv_adhoc_aes(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	if (strlen((const char *)respbuf) == header_len) {
		...
		/* SET operation */
		copy_len = (strlen((const char *)respbuf) - header_len);
...
woal_priv_tx_bf_cfg(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
	...
	char_count = strlen((const char *)respbuf) - header_len;
	copy_len = char_count;
	memset(buf, 0, sizeof(buf));
	if (char_count) {
		if (copy_len > sizeof(buf)) {
		...
		if (woal_atoi(&bf_action, (char *)&buf[0]) != MLAN_STATUS_SUCCESS) {
			ret = -EINVAL;
			goto done;
		}
		switch (bf_action) {
		case BF_GLOBAL_CONFIGURATION:
			if (char_count == 1) {
				...
				for (i = 1, str = &buf[2]; token != NULL; i++) {
					token = (t_u8 *)strstr((const char *)str, " ");
					pos = str;
					if (token != NULL) {
						*token = '\0';
						str = token + 1;
					}
					woal_atoi(&tmp_val, (char *)pos);
					switch (i) {
		...
		case TRIGGER_SOUNDING_FOR_PEER:
			...
			woal_mac2u8(bf_sound->peer_mac, (char *)&buf[2]);
			action = MLAN_ACT_SET;
			bf_cfg.action = BF_CFG_ACT_SET;
			break;
		case SET_GET_BF_PERIODICITY:
			...
			woal_mac2u8(bf_periodicity->peer_mac, (char *)&buf[2]);
			if (char_count == 19) {
				action = MLAN_ACT_GET;
				bf_cfg.action = BF_CFG_ACT_GET;
			} else {
				action = MLAN_ACT_SET;
				bf_cfg.action = BF_CFG_ACT_SET;
				if (woal_atoi(&interval, (char *)&buf[20]) !=
				    MLAN_STATUS_SUCCESS) {
					ret = -EINVAL;
					goto done;
				}
		...
		case TX_BF_FOR_PEER_ENBL:
			...
			if (char_count == 1) {
				...
				woal_mac2u8(tx_bf_peer->peer_mac, (char *)&buf[2]);
				woal_atoi(&tmp_val, (char *)&buf[20]);
				tx_bf_peer->bf_enbl = (t_u8) tmp_val;
				woal_atoi(&tmp_val, (char *)&buf[22]);
				tx_bf_peer->sounding_enbl = (t_u8) tmp_val;
				woal_atoi(&tmp_val, (char *)&buf[24]);
		...
		case SET_SNR_THR_PEER:
			...
			if (char_count == 1) {
				...
				woal_mac2u8(bf_snr->peer_mac, (char *)&buf[2]);
				if (woal_atoi(&snr, (char *)&buf[20]) !=
				    MLAN_STATUS_SUCCESS) {
					ret = -EINVAL;
					goto done;
				}
	...
	if (action == MLAN_ACT_GET) {
		...
		case BF_GLOBAL_CONFIGURATION:
			...
			break;
		case SET_GET_BF_PERIODICITY:
			copy_len += sprintf((char *)buf + copy_len,
			...
			copy_len += sprintf((char *)buf + copy_len, "%c", ' ');
			copy_len +=
				sprintf((char *)buf + copy_len, "%d",
...
#define MLAN_BYPASS_PKT_EXTRA_OFFSET        (4)
	...
	copyLen = strlen((const char *)respbuf) - header_len;
...
#define CMD_BUF_LEN   2048
	...
	if (strncmp(buf, CMD_MARVELL, strlen(CMD_MARVELL)) == 0) {
		...
		if (strncmp
			...
			/* Get version */
			len = woal_get_priv_driver_version(priv, (t_u8 *)buf,
							   priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_setget_priv_bandcfg(priv, (t_u8 *)buf,
						       priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_hostcmd(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_HTTXCFG,
			    strlen(PRIV_CMD_HTTXCFG)) == 0) {
			/* Set/Get HT Tx configuration */
			len = woal_setget_priv_httxcfg(priv, (t_u8 *)buf,
						       priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_setget_priv_htcapinfo(priv, (t_u8 *)buf,
							 priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_setget_priv_addbapara(priv, (t_u8 *)buf,
							 priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_setget_priv_aggrpriotbl(priv, (t_u8 *)buf,
							   priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_setget_priv_addbareject(priv, (t_u8 *)buf,
							   priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Get data rate */
			len = woal_get_priv_datarate(priv, (t_u8 *)buf,
						     priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Set/Get tx rate cfg */
			len = woal_setget_priv_txratecfg(priv, (t_u8 *)buf,
							 priv_cmd.total_len);
			goto handled;
#ifdef STA_SUPPORT
		} else if (strncmp
			...
			len = woal_get_priv_getlog(priv, (t_u8 *)buf,
						   priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			len = woal_priv_customie(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_ESUPPMODE,
			    strlen(PRIV_CMD_ESUPPMODE)) == 0) {
			/* Esupplicant mode configuration */
			len = woal_setget_priv_esuppmode(priv, (t_u8 *)buf,
							 priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_setget_priv_passphrase(priv, (t_u8 *)buf,
							  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Deauth */
			len = woal_priv_deauth(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
#ifdef UAP_SUPPORT
		} else if (strncmp
			...
			/* AP Deauth */
			len = woal_priv_ap_deauth(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Get STA list */
			len = woal_priv_get_sta_list(priv, (t_u8 *)buf,
						     priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* BSS config */
			len = woal_priv_bss_config(priv, (t_u8 *)buf,
						   priv_cmd.total_len);
			goto handled;
#endif
#if defined(WIFI_DIRECT_SUPPORT)
			...
			/* BSS Role */
			len = woal_priv_bssrole(priv, (t_u8 *)buf,
			...
			/* Set user scan */
			len = woal_priv_setuserscan(priv, (t_u8 *)buf,
						    priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Get scan table */
			len = woal_priv_getscantable(priv, (t_u8 *)buf,
						     priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_extcapcfg(priv, (t_u8 *)buf,
						  (t_u32) priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			/* Deep sleep */
			len = woal_priv_setgetdeepsleep(priv, (t_u8 *)buf,
							priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* IP address */
			len = woal_priv_setgetipaddr(priv, (t_u8 *)buf,
						     (t_u32) priv_cmd.
						     total_len);
			goto handled;
		} else if (strncmp
			...
			/* WPS Session */
			len = woal_priv_setwpssession(priv, (t_u8 *)buf,
						      priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* OTP user data */
			len = woal_priv_otpuserdata(priv, (t_u8 *)buf,
						    priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Country code */
			len = woal_priv_set_get_countrycode(priv, (t_u8 *)buf,
							    priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* TCP ack enhancement */
			len = woal_priv_setgettcpackenh(priv, (t_u8 *)buf,
							priv_cmd.total_len);
			goto handled;
#ifdef REASSOCIATION
		} else if (strncmp
			...
			/* Associate to essid */
			len = woal_priv_assocessid(priv, (t_u8 *)buf,
						   priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			/* wakeup reason */
			len = woal_priv_getwakeupreason(priv, (t_u8 *)buf,
							priv_cmd.total_len);
			goto handled;
#ifdef STA_SUPPORT
		} else if (strncmp
			...
			/* Listen Interval */
			len = woal_priv_set_get_listeninterval(priv, (t_u8 *)buf,
							       priv_cmd.
							       total_len);
			goto handled;
#endif
			...
			len = woal_priv_set_get_drvdbg(priv, (t_u8 *)buf,
						       priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			/* HS configuration */
			len = woal_priv_hscfg(priv, (t_u8 *)buf, priv_cmd.total_len,
					      MTRUE);
			goto handled;
		} else if (strncmp
			...
			/* Set HS parameter */
			len = woal_priv_hssetpara(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Scan configuration */
			len = woal_priv_set_get_scancfg(priv, (t_u8 *)buf,
							priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Set bss mode */
			len = woal_priv_set_bss_mode(priv, (t_u8 *)buf,
						     priv_cmd.total_len);
			goto handled;
#ifdef STA_SUPPORT
		} else if (strncmp
			...
			/* Set essid */
			len = woal_priv_set_essid(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Set/Get PS mode */
			len = woal_priv_set_get_psmode(priv, (t_u8 *)buf,
						       priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			/* Performs warm reset */
			len = woal_priv_warmreset(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_txpowercfg(priv, (t_u8 *)buf,
						   priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* PS configurations */
			len = woal_priv_pscfg(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_SLEEPPD,
			    strlen(PRIV_CMD_SLEEPPD)) == 0) {
			/* Sleep period */
			len = woal_priv_sleeppd(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_TXCONTROL,
			    strlen(PRIV_CMD_TXCONTROL)) == 0) {
			/* Tx control */
			len = woal_priv_txcontrol(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_arpfilter(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			len = woal_priv_mgmt_frame_passthru_ctrl(priv, (t_u8 *)buf,
								 priv_cmd.
								 total_len);
			goto handled;
		} else if (strncmp
			...
			/* Queue config */
			len = woal_priv_qconfig(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_ADDTS,
			    strlen(PRIV_CMD_ADDTS)) == 0) {
			/* Send an ADDTS TSPEC */
			len = woal_priv_wmm_addts_req_ioctl(priv, (t_u8 *)buf,
							    priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Send a DELTS TSPE */
			len = woal_priv_wmm_delts_req_ioctl(priv, (t_u8 *)buf,
							    priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_wmm_queue_status_ioctl(priv, (t_u8 *)buf,
							       priv_cmd.
							       total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_wmm_ts_status_ioctl(priv, (t_u8 *)buf,
							    priv_cmd.total_len);
			goto handled;
#ifdef STA_SUPPORT
		} else if (strncmp
			...
			/* MAC CTRL */
			len = woal_priv_macctrl(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_GETWAP,
			    strlen(PRIV_CMD_GETWAP)) == 0) {
			/* Get WAP */
			len = woal_priv_getwap(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_REGION_CODE,
			    strlen(PRIV_CMD_REGION_CODE)) == 0) {
			/* Region Code */
			len = woal_priv_region_code(priv, (t_u8 *)buf,
						    priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Set FW MAC address */
			len = woal_priv_fwmacaddr(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
#if defined(WIFI_DIRECT_SUPPORT)
#ifdef STA_CFG80211
			...
			if (IS_STA_CFG80211(cfg80211_wext)) {
				/* Set offchannel */
				len = woal_priv_offchannel(priv, (t_u8 *)buf,
							   priv_cmd.total_len);
			} else
			...
			len = woal_priv_get_driver_verext(priv, (t_u8 *)buf,
							  priv_cmd.total_len);
			goto handled;
#if defined(STA_SUPPORT)
		} else if (strncmp
			...
			/* Set/Get radio */
			len = woal_priv_radio_ctrl(priv, (t_u8 *)buf,
						   priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			len = woal_priv_wmm_cfg(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
#if defined(STA_SUPPORT)
		} else if (strncmp
			...
			len = woal_priv_wws_cfg(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
#if defined(REASSOCIATION)
		} else if (strncmp
			...
			len = woal_priv_set_get_reassoc(priv, (t_u8 *)buf,
							priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			len = woal_priv_txbuf_cfg(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
#ifdef STA_SUPPORT
		} else if (strncmp
			...
			/* Set/Get auth type */
			len = woal_priv_auth_type(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			len = woal_priv_11h_local_pwr_constraint(priv, (t_u8 *)buf,
								 priv_cmd.
								 total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_ht_stream_cfg(priv, (t_u8 *)buf,
						      priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Get thermal reading */
			len = woal_priv_thermal(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_BCN_INTERVAL,
			    strlen(PRIV_CMD_BCN_INTERVAL)) == 0) {
			/* Set/Get beacon interval */
			len = woal_priv_beacon_interval(priv, (t_u8 *)buf,
							priv_cmd.total_len);
			goto handled;
#ifdef STA_SUPPORT
		} else if (strncmp
			...
			/* Get signal */
			len = woal_priv_get_signal(priv, (t_u8 *)buf,
						   priv_cmd.total_len);
			goto handled;
#endif
#if defined(STA_SUPPORT)
			...
			/* Configure PMF */
			len = woal_priv_set_get_pmfcfg(priv, (t_u8 *)buf,
						       priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			len = woal_priv_inactivity_timeout_ext(priv, (t_u8 *)buf,
							       priv_cmd.
							       total_len);
			goto handled;
		} else if (strncmp
			...
			/* Set/Get ATIM window */
			len = woal_priv_atim_window(priv, (t_u8 *)buf,
						    priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_11n_amsdu_aggr_ctrl(priv, (t_u8 *)buf,
							    priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_tx_bf_cap_ioctl(priv, (t_u8 *)buf,
							priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_sdio_clock_ioctl(priv, (t_u8 *)buf,
							 priv_cmd.total_len);
			goto handled;
#if defined(SDIO_MULTI_PORT_TX_AGGR) || defined(SDIO_MULTI_PORT_RX_AGGR)
		} else if (strncmp
			...
			len = woal_priv_sdio_mpa_ctrl(priv, (t_u8 *)buf,
						      priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			len = woal_priv_sleep_params_ioctl(priv, (t_u8 *)buf,
							   priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_net_monitor_ioctl(priv, (t_u8 *)buf,
							  priv_cmd.total_len);
			goto handled;
#if defined(DFS_TESTING_SUPPORT)
		} else if (strncmp
			...
			len = woal_priv_dfs_testing(priv, (t_u8 *)buf,
						    priv_cmd.total_len);
			goto handled;
#endif
		} else if (strncmp
			...
			len = woal_priv_cfp_code(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_ANT_CFG,
			    strlen(PRIV_CMD_ANT_CFG)) == 0) {
			/* Set/Get Tx/Rx antenna */
			len = woal_priv_set_get_tx_rx_ant(priv, (t_u8 *)buf,
							  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_sysclock(priv, (t_u8 *)buf, priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			   (buf + strlen(CMD_MARVELL), PRIV_CMD_ADHOC_AES,
			    strlen(PRIV_CMD_ADHOC_AES)) == 0) {
			/* Adhoc AES control */
			len = woal_priv_adhoc_aes(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Associate to a specific indexed entry in the
			   ScanTable */
			len = woal_priv_associate_ssid_bssid(priv, (t_u8 *)buf,
							     priv_cmd.
							     total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_tx_bf_cfg(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_cmd53rdwr(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_port_ctrl(priv, (t_u8 *)buf,
						  priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			/* Private IOCTL entry to get the By-passed TX packet
			   from upper layer */
			len = woal_priv_bypassed_packet(priv, (t_u8 *)buf,
							priv_cmd.total_len);
			goto handled;
		} else if (strncmp
			...
			len = woal_priv_coalescing_status(priv, (t_u8 *)buf,
							  priv_cmd.total_len);
			goto handled;
#if defined(WIFI_DIRECT_SUPPORT)
#endif
	...
	if (strncmp(buf, "RSSILOW-THRESHOLD", strlen("RSSILOW-THRESHOLD")) == 0) {
		...
		if (MLAN_STATUS_SUCCESS !=
		    woal_set_scan_cfg(priv, (char *)buf, priv_cmd.total_len)) {
			ret = -EFAULT;
			goto done;
		}
		len = sprintf((char *)buf, "OK\n") + 1;
		...
		if (bss_info.media_connected) {
			...
			len = sprintf((char *)buf, "%.32s rssi %d\n",
				      bss_info.ssid.ssid,
		...
		len = sprintf((char *)buf, "LinkSpeed %d\n",
			      (int)(rate.rate * 500000 / 1000000))
			+ 1;
	} else
#endif
	if (strncmp(buf, "MACADDR", strlen("MACADDR")) == 0) {
		len = sprintf((char *)buf, "Macaddr = %02X:%02X:%02X:%02X:%02X:%02X\n",
		...
		len = sprintf((char *)buf, "powermode = %d\n", power_mode) + 1;
		...
		if (MLAN_STATUS_SUCCESS !=
		    woal_request_country_power_table(priv, (char *)country_code)) {
			ret = -EFAULT;
			goto done;
		}
#ifdef STA_CFG80211
...
#endif
		if (MLAN_STATUS_SUCCESS !=
		    woal_set_region_code(priv, (char *)country_code)) {
			ret = -EFAULT;
			goto done;
		}
		len = sprintf((char *)buf, "OK\n") + 1;
		...
		if (MLAN_STATUS_SUCCESS != woal_set_combo_scan(priv, (char *)buf,
							       priv_cmd.
							       total_len)) {
			ret = -EFAULT;
			goto done;
		...
		ret = woal_uap_set_ap_cfg(priv, (t_u8 *)pdata,
					  priv_cmd.total_len -
					  strlen("AP_SET_CFG") - 1);
		if (ret)
			goto done;
		...
		if (MLAN_STATUS_SUCCESS !=
		    woal_set_bg_scan(priv, (char *)buf, priv_cmd.total_len)) {
			ret = -EFAULT;
			goto done;
		}
		...
		len = sprintf((char *)buf, "EVENT=%d\n", priv->last_event) + 1;
		priv->last_event = 0;
	} else if (strncmp(buf, "GET_802_11W", strlen("GET_802_11W")) == 0) {
		len = sprintf((char *)buf, "802_11W=ENABLED\n") + 1;
	}
#endif /* STA_CFG80211 */
		...
		*buf = 0;
		len = 1;
	} else if (strncmp(buf, "MIRACAST", strlen("MIRACAST")) == 0) {
...
#endif
		len = sprintf((char *)buf, "OK\n") + 1;
	} else if (strncmp(buf, "SCAN_TIMING", strlen("SCAN_TIMING")) == 0) {
#ifdef WIFI_DIRECT_SUPPORT
#if defined(STA_CFG80211) || defined(UAP_CFG80211)
#endif
#endif
		len = sprintf((char *)buf, "OK\n") + 1;
	} else if (strncmp(buf, "BA_WSIZE_RX", strlen("BA_WSIZE_RX")) == 0) {
		pdata = buf + strlen("BA_WSIZE_RX") + 1;
		len = priv_cmd.total_len - strlen("BA_WSIZE_RX") - 1;
		if (MLAN_STATUS_SUCCESS !=
		    woal_set_rx_ba_winsize(priv, (t_u8 *)pdata, len)) {
			ret = -EFAULT;
			goto done;
		}
		len = sprintf((char *)buf, "OK\n") + 1;
	} else if (strncmp(buf, "BA_WSIZE_TX", strlen("BA_WSIZE_TX")) == 0) {
		pdata = buf + strlen("BA_WSIZE_TX") + 1;
		len = priv_cmd.total_len - strlen("BA_WSIZE_TX") - 1;
		if (MLAN_STATUS_SUCCESS !=
		    woal_set_tx_ba_winsize(priv, (t_u8 *)pdata, len)) {
			ret = -EFAULT;
			goto done;
		}
		len = sprintf((char *)buf, "OK\n") + 1;
