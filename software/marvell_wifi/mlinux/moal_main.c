		     ...
		     t_u8 * offset_string, t_u8 * value_string)
{
	...
	if (MLAN_STATUS_SUCCESS != woal_atoi(&type, (char *)type_string))
		goto done;
	if (MLAN_STATUS_SUCCESS != woal_atoi(&offset, (char *)offset_string))
		goto done;
	if (MLAN_STATUS_SUCCESS != woal_atoi(&value, (char *)value_string))
		goto done;
...
woal_process_init_cfg(moal_handle * handle, t_u8 * data, t_size size)
{
	...
	while ((line_len = parse_cfg_get_line(data, size, s)) != -1) {
		...
		/* Process MAC addr */
		if (strncmp((const char *)pos, "mac_addr", 8) == 0) {
			intf_s = (t_u8 *)strchr((const char *)pos, '=');
			if (intf_s != NULL)
				intf_e = (t_u8 *)strchr((const char *)intf_s, ':');
			else
				intf_e = NULL;
			if (intf_s != NULL && intf_e != NULL) {
				strncpy((char *)bss_mac_addr, (const char *)intf_e + 1,
					MAX_MAC_ADDR_LEN - 1);
				...
				strncpy((char *)bss_mac_name, (const char *)intf_s + 1,
					intf_e - intf_s - 1);
				bss_mac_name[intf_e - intf_s - 1] = '\0';
				for (i = 0; i < handle->priv_num; i++) {
					if (strcmp
					    ((const char *)bss_mac_name,
		...
		/* Process REG value */
		else if (strncmp((const char *)pos, "wlan_reg", 8) == 0) {
			intf_s = (t_u8 *)strchr((const char *)pos, '=');
			if (intf_s != NULL)
				intf_e = (t_u8 *)strchr((const char *)intf_s, ',');
			else
			...
			if (intf_s != NULL && intf_e != NULL) {
				/* Copy type */
				strncpy((char *)type, (const char *)intf_s + 1, 1);
				type[1] = '\0';
			} else {
				PRINTM(MERROR, "Wrong config file format %d\n",
				       __LINE__);
			...
			if (intf_e != NULL) {
				...
				/* Copy offset */
				strncpy((char *)offset, (const char *)intf_s, intf_e - intf_s);
			...
			/* Copy value */
			strncpy((char *)value, (const char *)intf_s, strlen((const char *)intf_s));
		      ...
		      mlan_buffer * pmbuf, struct sk_buff *skb)
{
	...
	if (IS_SKB_MAGIC_VLAN(skb)) {
		...
		case __constant_htons(ETH_P_IP):
            ...
            tid = IPTOS_PREC(iph->tos) >> IPTOS_OFFSET;
...
woal_init_priv(moal_private * priv, t_u8 wait_option)
{
	...
	priv->enable_tcp_ack_enh = MFALSE/*MTRUE*/;
...
//		remove_wait_queue(&pmoal_thread->wait_q, &wait);
        wait_event_interruptible(pmoal_thread->wait_q, 0);
...
woal_send_disconnect_to_system(moal_private * priv)
{
	...
	strncpy((char *)event_buf, CUS_EVT_AP_CONNECTED,
...
woal_dump_firmware_info(moal_handle * phandle)
{
	...
	int ret = 0;
	unsigned int reg, reg_start, reg_end;
	char path_name[64], file_name[32];
	t_u8 *ITCM_Ptr = NULL;
	t_u8 *DTCM_Ptr = NULL;
	t_u8 *SQRAM_Ptr = NULL;
	t_u8 *dbg_ptr = NULL;
	...
	if (mac_addr) {
		...
		if (len < sizeof(temp)) {
			...
			/* note: the following function overwrites the temp
			   buffer */
			woal_mac2u8(handle->mac_addr, (char *)temp);
		}
	}
...
#if 1//LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 14)
